...
class GWDataset(Dataset):
    ...
    def __init__(self,
        ...
        split_ids = np.load(os.path.join(gw_root_dir, 'almazan_cv_indices.npy'))

        # load the dataset
        word_list = DatasetLoader.load_gw(path = gw_root_dir)
        
        self.train_list = [word for word, split_id
                                in zip(word_list, split_ids)
                                if split_id != split_idx]
        
        self.test_list = [word for word, split_id
                                in zip(word_list, split_ids)
                                if split_id == split_idx]
