...
class IAMDataset(Dataset):
    '''
    PyTorch dataset class for the segmentation-based IAM dataset
    '''
    ...
    def mainLoader(self, partition=None, transforms=HomographyAugmentation()):
        ...
        if partition == 'test':
            ...
            # find test words with single occurences
            test_strings = [word.get_transcription() for word in self.test_list]
            unique_word_strings, counts = np.unique(test_strings,
                                                return_counts=True)
            
            # queries = testset - stopwords - singles
            qry_word_strings = unique_word_strings[np.where(counts > 1)[0]]
            qry_word_strings = [word for word in qry_word_strings if word not in stop_words]

            query_list = np.zeros(len(self.test_list), np.int8)
            qry_ids = [i for i in range(len(self.test_list)) if
                       test_strings[i] in qry_word_strings]
