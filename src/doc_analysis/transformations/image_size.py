...
def check_size(img, min_image_width_height, fixed_image_size=None):
    ...
    if fixed_image_size is not None:
        ...
        new_img = resize(image=img, output_shape=fixed_image_size[::-1], mode='constant', anti_aliasing=True)
        ...
        new_img = resize(image=img, output_shape=new_shape, mode='constant', anti_aliasing=True)
        new_img = new_img.astype(np.float32)
        return new_img
    else:
        return img