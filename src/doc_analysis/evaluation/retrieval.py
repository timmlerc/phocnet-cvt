...
def map_from_query_test_feature_matrices(query_features, test_features,
    ...
    #if metric == 'dap':
    #    result_probabilities = np.zeros((test_features.shape[0],query_features.shape[0]))
    #    _dap.dap_retrieval_py(query_features.astype(np.double), test_features.astype(np.double), result_probabilities)
    #
    #    c_prob = result_probabilities.T
    #    retrieval_indices = np.argsort(c_prob, axis=1)
